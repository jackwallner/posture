...
#
# If you already have build/Posture.xcarchive:
...
if [[ -f "$PROJECT_YML" ]]; then
    ...
    if [[ -n "$CURRENT_BUILD" && "$CURRENT_BUILD" =~ ^[0-9]+$ ]]; then
        ...
        git add "$PROJECT_YML" \
            && git commit -m "chore(posture): bump build $CURRENT_BUILD -> $NEW_BUILD" || true
...
ARCHIVE="$ROOT/build/Posture.xcarchive"
rm -rf "$ARCHIVE"

echo "==> Archive (Release)"
xcodebuild -project Posture.xcodeproj \
  -scheme Posture \
