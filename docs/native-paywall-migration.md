...
## Posture-specific notes

Custom paywall impression ids (sheet entry points — not session-deduped):

- `posture_settings_sheet` — Settings Pro postcard
- `posture_history_sheet` — History look-back upsell

Implementation: `Posture/Views/PaywallView.swift`, `Shared/Services/SubscriptionService.swift`.
