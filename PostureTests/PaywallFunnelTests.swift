...
final class PaywallFunnelTests: XCTestCase {
    ...
    func testCountsPitchesPerSurfaceAndInTotal() {
        ConversionDiagnostics.recordPitchView(impressionID: "posture_pro_tab")
        ConversionDiagnostics.recordPitchView(impressionID: "posture_pro_tab")
        ConversionDiagnostics.recordPitchView(impressionID: "posture_level_gate")

        let attributes = ConversionDiagnostics.subscriberAttributes
        XCTAssertEqual(attributes["pitch_views_total"], "3")
        XCTAssertEqual(attributes["pitch_views_pro_tab"], "2")
        XCTAssertEqual(attributes["pitch_last"], "level_gate")
    }
    ...
    func testConversionFreezesTheSurfaceAndCountAtTheMomentOfSale() {
        ...
        let attributes = ConversionDiagnostics.subscriberAttributes
        XCTAssertEqual(attributes["converted_surface"], "level_gate")
    ...
    func testAttributeKeysStayInsideRevenueCatsLimit() {
        ...
        ConversionDiagnostics.recordPitchView(impressionID: "posture_\(long)")
